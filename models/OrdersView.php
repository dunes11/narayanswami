...
class OrdersView extends Orders
{
    ...
    protected function setupOtherOptions()
    {
        ...
        */
        ...
        if (!$this->ShowMultipleDetails) { // Skip record count if show multiple details
            $body .= "&nbsp;" . str_replace("%c", Container("orderdetails")->Count, $Language->phrase("DetailCount"));
        }
        ...
        $detailTbl->Count = $detailTbl->loadRecordCount($detailFilter);
        if (!$this->ShowMultipleDetails) { // Skip record count if show multiple details
            $body .= "&nbsp;" . str_replace("%c", Container("order_details_extended")->Count, $Language->phrase("DetailCount"));
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fordersview" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-key="' . HtmlEncode(ArrayToJsonAttribute($this->RecKey)) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
        ...
        if (Config("EXPORT_DETAIL_RECORDS") && in_array("orderdetails", explode(",", $this->getCurrentDetailTable()))) {
            ...
            if ($rsdetail) {
                ...
                if (!$this->isExport("csv") || Config("EXPORT_DETAIL_RECORDS_FOR_CSV")) {
                    ...
                    $orderdetails->exportDocument($doc, new Recordset($rsdetail), 1, $detailcnt);
                    $doc->setTable($oldtbl);
                }
        ...
        if (Config("EXPORT_DETAIL_RECORDS") && in_array("order_details_extended", explode(",", $this->getCurrentDetailTable()))) {
            ...
            if ($rsdetail) {
                ...
                if (!$this->isExport("csv") || Config("EXPORT_DETAIL_RECORDS_FOR_CSV")) {
                    ...
                    $order_details_extended->exportDocument($doc, new Recordset($rsdetail), 1, $detailcnt);
                    $doc->setTable($oldtbl);
                }
                $doc->setStyle($exportStyle); // Restore
            }
