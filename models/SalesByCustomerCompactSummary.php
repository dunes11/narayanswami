...
class SalesByCustomerCompactSummary extends SalesByCustomerCompact
{
    ...
    public function __construct()
    {
        ...
        if (!isset($GLOBALS["Sales_By_Customer_Compact"]) || get_class($GLOBALS["Sales_By_Customer_Compact"]) == PROJECT_NAMESPACE . "Sales_By_Customer_Compact") {
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_SEARCH) { // Search row
            ...
            $this->ExtendedPrice->CellAttrs["class"] = ($this->RowTotalType == ROWTOTAL_PAGE || $this->RowTotalType == ROWTOTAL_GRAND) ? "ew-rpt-grp-aggregate" : (($this->RowGroupLevel < 2) ? "ew-rpt-grp-summary-" . $this->RowGroupLevel : (($this->LastGroupCount % 2 != 1) ? "ew-table-alt-row" : ""));
            ...
            if (!$this->isExport() && AllowList(PROJECT_ID . $this->ExtendedPrice->DrillDownTable)) {
                ...
                $parm = ($this->RowTotalType == ROWTOTAL_GROUP && $this->RowGroupLevel >= 2) ? 0 : -1;
                $drillurl = str_replace("=f1", "=" . Encrypt($this->getDrillDownSql($this->OrderID, "OrderID", $this->RowTotalType, $parm)), $drillurl);
                $this->ExtendedPrice->LinkAttrs["title"] = JsEncodeAttribute($GLOBALS["Language"]->phrase("ClickToDrillDown"));
                $this->ExtendedPrice->LinkAttrs["class"] = "ew-drill-link";
                $this->ExtendedPrice->LinkAttrs["data-drilldown-placement"] = "bottom";
            ...
            if (!$this->isExport() && AllowList(PROJECT_ID . $this->ExtendedPrice->DrillDownTable)) {
                $drillurl = $this->ExtendedPrice->DrillDownUrl;
                $drillurl = str_replace("=f0", "=" . Encrypt($this->getDrillDownSql($this->CompanyName, "CompanyName", 0)), $drillurl);
                $drillurl = str_replace("=f1", "=" . Encrypt($this->getDrillDownSql($this->OrderID, "OrderID", 0)), $drillurl);
                $this->ExtendedPrice->LinkAttrs["title"] = JsEncodeAttribute($GLOBALS["Language"]->phrase("ClickToDrillDown"));
                $this->ExtendedPrice->LinkAttrs["class"] = "ew-drill-link";
                $this->ExtendedPrice->LinkAttrs["data-drilldown-placement"] = "bottom";
                $this->ExtendedPrice->LinkAttrs->merge(DrillDownAttributes($drillurl, 'Sales_By_Customer_Compact_ExtendedPrice', $GLOBALS["Language"]->tablePhrase('Sales_By_Order', 'TblCaption'), $this->UseDrillDownPanel));
