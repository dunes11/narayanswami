...
class ProductsList extends Products
{
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fproductssrch", $filters);
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fproductslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . " " . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fproductslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . " " . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("ProductName", $this->createColumnOption("ProductName"));
            $option->add("SupplierID", $this->createColumnOption("SupplierID"));
            $option->add("CategoryID", $this->createColumnOption("CategoryID"));
            $option->add("QuantityPerUnit", $this->createColumnOption("QuantityPerUnit"));
            $option->add("UnitPrice", $this->createColumnOption("UnitPrice"));
            $option->add("UnitsInStock", $this->createColumnOption("UnitsInStock"));
            $option->add("UnitsOnOrder", $this->createColumnOption("UnitsOnOrder"));
            $option->add("ReorderLevel", $this->createColumnOption("ReorderLevel"));
            $option->add("Discontinued", $this->createColumnOption("Discontinued"));
            $option->add("EAN13", $this->createColumnOption("EAN13"));
        }
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->ProductName->UseFilter && !EmptyValue($this->ProductName->AdvancedSearch->SearchValue)) {
                if (is_array($this->ProductName->AdvancedSearch->SearchValue)) {
                    $this->ProductName->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->ProductName->AdvancedSearch->SearchValue);
                }
                $this->ProductName->EditValue = explode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->ProductName->AdvancedSearch->SearchValue);
            }
